package com.isabel.reviewmovie;
...
public class Camera extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener, View.OnClickListener{
    ...
    private void registerService() {
        GoogleSignInOptions options= new GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN).requestEmail().requestId().build();
        api= new GoogleApiClient.Builder(this).enableAutoManage(this, this).addApi(Auth.GOOGLE_SIGN_IN_API, options).build();
    ...
    private void loadData(GoogleSignInResult result) {
        if (result.isSuccess()){
            ...
            Toast.makeText(this, "error no conoce el password", Toast.LENGTH_SHORT).show();
        }
    }
}
